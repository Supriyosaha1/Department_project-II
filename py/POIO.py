...
class mockobs(object):
    def __init__(self,Dir,FileName,ICFile,load_spectrum=False,load_image=False,load_cube=False,unit_l_arcsec=1.0,lumdist_cm=1.0,redshift=0.0,cube_h5_file=None,ndirection=1):
        self.Dir      = Dir
        ...
        if load_image:
            self.imsize_cu, self.image_npix, self.imtot = self.__read_image(index=ndirection)
            self.imtot = self.imtot * self.nPhotPerPacket # [photons / s]

        if load_spectrum:  # restframe spectrum
            self.spec_lmin,self.spec_lmax,self.spec_npix,self.spec = self.__read_spectrum(index=ndirection)
        ...
        if load_cube:  # obs-frame cube
            if cube_h5_file is not None:
                ...
                self.cube_nlbda,self.cube_nxy,self.cube_lmin,self.cube_lmax,self.cube_imsize,self.cube = self.__read_cube(index=ndirection)
                ...
                l = np.arange(self.cube_lmin,self.cube_lmax-0.00001,dl) + 0.5*dl  # [A]
                self.cube_lbda_Angstrom  = l 
                self.cube_dlbda_Angstrom = dl
                l = l * 1e-8  # [cm]
                self.cube = self.cube * self.nPhotPerPacket      # [photons / s]
                self.cube = self.cube * ly.h_cgs * ly.clight / l # [erg / s]
                self.cube = self.cube / dl                       # [erg / s / A]
...
#                 # Else : [erg / s / A / arcsec2 / phot packet],   and stays in luminosity all along
#                 self.cube = energy * self.cube   # [erg / s / A / arcsec2 / cm2 ]
    ...
    def __read_image(self,index=1):
        from scipy.io import FortranFile as ff
        f = ff('%s/%s_image'%(self.Dir,self.FileName))
        n = f.read_ints()[0]
        imsize = f.read_reals('d')[0]
        imcenter = f.read_reals('d')
        im = np.zeros((index,n,n))
        im1 = f.read_reals('d')
        im[0] = im1.reshape((n,n))
        for j in range(1,index):
            f.read_ints()
            f.read_reals('d')
            f.read_reals('d')
            im1 = f.read_reals('d')
            im[j]=im1.reshape((n,n))
        f.close()
    ...
    def __read_spectrum(self,index=1):
        from scipy.io import FortranFile as ff
        f = ff('%s/%s_spectrum'%(self.Dir,self.FileName))
        n = f.read_ints()[0]
        aperture,lmin,lmax = f.read_reals('d')
        spec = np.zeros((index,n))
