program HaPhotonsFromGas
  ...
  real(kind=8)              :: tcool_resolution = 0.0d0 ! ignore collisional emission from cells with Dt * tcool_resolution > tcool  
  ! --- miscelaneous
  ...
  if(narg .lt. 1)then
     write(*,*)'You should type: HaPhotonsFromGas params.dat'
     write(*,*)'File params.dat should contain a parameter namelist'
     stop
  end if
  call get_command_argument(1, parameter_file)
  call read_HaPhotonsFromGas_params(parameter_file)
  if (verbose) call print_HaPhotonsFromGas_params
  ! ----------------------------------------------------------------------------
  ...
  call ramses_get_HaEmiss_HIDopwidth(repository,snapnum,nleaftot,nvar,ramses_var,recomb_em,coll_em,HIDopWidth,sample=emitting_cells)
  ...
  nu_HA = clight / lambda_HA_Ang * cmtoA
  recomb_total = sum(recomb_em) / (planck*nu_HA)  ! nb of photons per second
  coll_total   = sum(coll_em) / (planck*nu_HA)  ! nb of photons per second
  
  print*,'coll_total,recomb_total [erg/s] = ',coll_total*(planck*nu_HA),recomb_total*(planck*nu_HA)
     ...
     do iphot = 1,nphotons
        ...
        do while (.not. ok) 
           ...
           if (domain_contains_point(x,emission_domain)) then 
              ...
              nu_cell(iphot) = (HIDopWidth(ilow) * nu_HA / clight) * nu + nu_HA
...
contains
    ...
    subroutine read_HaPhotonsFromGas_params(pfile)
    ...
    do
       ...
       if(err/=0) exit
       if (line(1:19) == '[HaPhotonsFromGas]') then
  ...
  end subroutine read_HaPhotonsFromGas_params

  
  subroutine print_HaPhotonsFromGas_params(unit)
    ...
    if (present(unit)) then 
       write(unit,'(a,a,a)')         '[HaPhotonsFromGas]'
  ...
  end subroutine print_HaPhotonsFromGas_params


  
end program HaPhotonsFromGas
