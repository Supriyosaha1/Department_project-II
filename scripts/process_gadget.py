#!/usr/bin/env python3
"""
Process Gadget simulation output for Rascas analysis

This script processes Gadget output files and prepares them for analysis
with Rascas tools.
"""
...
from rascas_analysis.gadget import GadgetDataProcessor
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process Gadget output for Rascas analysis')
    parser.add_argument('data_path', help='Path to Gadget output directory')
    ...
    parser.add_argument('--convert-hdf5', action='store_true', 
                       help='Convert snapshots to HDF5 format')
    ...
    logger.info("Starting Gadget data processing")
    
    # Load configuration
    config_manager = ConfigManager(args.config) if args.config else ConfigManager()
    
    # Initialize Gadget processor
    try:
        processor = GadgetDataProcessor(args.data_path, config_manager.get_section('gadget'))
        logger.info(f"Initialized Gadget processor for: {args.data_path}")
    except Exception as e:
        logger.error(f"Failed to initialize Gadget processor: {e}")
        return 1
    ...
    for snapshot_id in snapshots_to_process:
        try:
            ...
            if args.convert_hdf5:
                hdf5_file = output_path / f"snapshot_{snapshot_id}" / f"snapshot_{snapshot_id}.h5"
                hdf5_file.parent.mkdir(parents=True, exist_ok=True)
                processor.convert_to_hdf5(snapshot_data, hdf5_file)
